import os
import uuid
dockerjdkversion="dockerjdkversion"
symbol = "openjdk:11.0.10-jdk"
infile = open(dockerjdkversion, mode="r", encoding="utf-8")
currentversion=infile.readline()
infile.close()
currentversion=currentversion.split("=")[-1]#openjdk:11.0.10-jdk
...
infile.close()
outfile = open(dockerjdkversion, mode="w", encoding="utf-8")
for line in lines:
    if (str(line).count(":") > 0):
